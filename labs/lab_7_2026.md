...
sample to the bam file. Some variant calling programs require read group information, or they don't run.
...
to figure out where the readgroup information is stored. Hint: You need to use samtools view -h to see the header. 
...
1. The server we are using has an online version of Rstudio. Go to this linke (https://indri.rcs.uvic.ca). 
...
scp YOURNAME@indri.rcs.uvic.ca:/project/biol470-grego/2026/YOUR_NAME/lab_7/lab_7.fb.Q20.vcf Desktop/
```
...
```R
vcf_file <- "/project/biol470-grego/2026/YOUR_NAME/lab_7/lab_7.fb.Q20.vcf"
vcf <- read.vcfR(vcf_file)
#extract the genotypes
gt_data <- vcfR2tidy(vcf, format_fields = 'GT' )
gt = gt_data$gt
names(gt) = c('Chr','Position','X','ID','Genotype','Allele')
gt
```
```output
# A tibble: 620 × 5
