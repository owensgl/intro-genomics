...
numbers. We can do this using 'bcftools annotate'.

```bash
module load bcftools
#Create a map file that links a number to each chromosome/contig. 
#In this script, we're grabbing all the contig names from the vcf header, and then printing out a number beside each which counts up.
bcftools view -h chinook.g5.m2.c5.d5.20miss.subsample.vcf.gz | grep "##contig" | sed -e 's/.*ID=\([^,>]*\).*/\1/' | awk '{print $1, NR}' > chinook.g5.m2.c5.d5.20miss.subsample.chr_map.txt
#Use bcftools annotate to replace the chromosomes with numbers
bcftools annotate --rename-chrs chinook.g5.m2.c5.d5.20miss.subsample.chr_map.txt chinook.g5.m2.c5.d5.20miss.subsample.vcf.gz -o chinook.g5.m2.c5.d5.20miss.subsample.nchrs.vcf
```
...
(chinook.g5.m2.c5.d5.20miss.subsample.chr_map.txt). Lets repeat the same plink steps and
try admixture again.
...
````
Don't worry if you don't get the exact same likelihood values in the output. Admixture is
stochastic and doesn't always produce the same answer.
You now have two output files: chinook.g5.m2.c5.d5.20miss.subsample.nchrs.1.Q and 
...
Your CV error may be slightly different than mine.
...
This looks interesting! This is an admixture graph. Each column is a separate sample and the colour
represents the proportion of it's genome from each ancestry group. 
We're sorting the x axis by the population label, so 
samples are ordered by population. We can see that the first half samples are mostly group 1
and the second half are mostly group 2 (or the reverse in your data). This is evidence that those populations reflect
...
tmp <- all_data %>%
  filter(k == 2, Q == "Q1") %>%
  select(sample.id, rank = value)

all_data %>%
  filter(k == 2) %>%
  inner_join(tmp) %>%
  ggplot(.,aes(x=fct_reorder(sample.id,rank),y=value,fill=factor(Q))) + 
  geom_bar(stat="identity",position="stack") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
-->
## Questions
