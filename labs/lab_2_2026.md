...
- Reinforce skills we learned last week for navigating command line.
...
Before we start creating new text files, lets talk about naming.
...
In this case,
the directory name we use is the special directory name `.` that means the current directory we're in.
