...
Before we trust our peaks, we need to ensure our model isn't suffering from "genomic inflation"—where population structure or cryptic relatedness causes p-values to be systematically lower than they should be. We quantify this using the Genomic Inflation Factor lambda. An ideal lambda is 1.0.
```R
# Calculate the expected p-values (uniform distribution)
n_snps <- nrow(gwas)
gwas <- gwas %>%
  arrange(P) %>%
...
![GWAS plot 2](../figs/lab_9.2.2.png)

### Question

- How many SNPs in your analysis remain significant after applying the Bonferroni correction?
- If you had used the standard p < 0.05 (which is 1.3 on the -log10 scale), how many "false positives" might you have reported?
...
although in this case, neither find any significant SNPs.
...
Even though nothing is genome-wide significant, we see a pretty big peak at the right side of the graph, as well as a
